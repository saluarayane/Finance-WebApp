...
interface ExpenseAnalysisProps {
  ...
  onExpensesDataLoad?: (fixed: FixedExpense[], variable: VariableExpense[]) => void;
  totalIncome?: number;
...
export function ExpenseAnalysis({ onUpdateBalance, onUpdateExpenses, onExpensesDataLoad, totalIncome = 23270.50, selectedMonth = "Mai" }: ExpenseAnalysisProps) {
  ...
  const categoryTotals = variableCategories.map(cat => ({
    ...cat,
    spent: todayExpenses.filter(exp => exp.category === cat.id).reduce((sum, exp) => sum + exp.amount, 0)
  }));

  const handleFormatCurrency = (value: string) => {
    value = value.replace(/\D/g, "");
    if (value) {
      return (parseInt(value) / 100).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
    return "";
  };
    ...
    <div className="space-y-6 w-full">
      <GlassCard className="p-5">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">Gastos Variáveis</h2>
          {todayExpenses.length > 0 && (
            <button onClick={() => setShowTodayExpenses(!showTodayExpenses)} className={clsx("p-2 rounded-lg transition-all", showTodayExpenses ? "bg-fuchsia-500/20 text-fuchsia-400" : "bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/10")}>
              <Edit3 size={16} />
            </button>
          )}
        </div>
        ...
        <div className="mb-5 bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-sm">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input type="text" inputMode="numeric" value={quickExpense} onChange={handleExpenseInput} placeholder="0,00" className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-9 pr-3 text-white outline-none focus:border-fuchsia-500/50 focus:ring-1 focus:ring-fuchsia-500/50 transition-all placeholder:text-white/20" />
            </div>
          </div>
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {variableCategories.map(category => {
                    ...
                    const isOrange = category.id === "alimentacao"; const isPurple = category.id === "lazer"; const isPink = category.id === "transporte"; const isCyan = category.id === "outros";
                    return (
                      <button key={category.id} onClick={() => handleCategorySelect(category.id)} className={clsx("py-2.5 px-3 rounded-xl border-2 transition-all backdrop-blur-md flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10", isOrange && "border-orange-500/60 text-orange-400", isPurple && "border-fuchsia-500/60 text-fuchsia-400", isPink && "border-pink-500/60 text-pink-400", isCyan && "border-cyan-500/60 text-cyan-400")}>
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-4 space-y-2 overflow-hidden">
                <div className="bg-white/5 border border-white/10 rounded-xl p-3 backdrop-blur-sm space-y-2">
                  {todayExpenses.map(expense => {
                    ...
                    return (
                      <div key={expense.id} className="flex items-center justify-between bg-white/5 border border-white/10 rounded-lg p-2.5">
                        <div className="text-xs text-white/70 font-medium flex-1 text-left">{category?.name}</div>
                        <div className="text-sm font-bold text-white flex-1 text-center">R$ {expense.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                        <div className="flex-1 flex justify-end">
                          <button onClick={() => handleRemoveTodayExpense(expense.id)} className="p-1.5 rounded-lg hover:bg-red-500/20 text-red-400 transition-all"><Trash2 size={14} /></button>
                        </div>
                      </div>
                    );
                  })}
            ...
            return (
              <div key={category.id} className="space-y-2">
                ...
                <div className="h-2.5 w-full bg-white/5 rounded-full overflow-hidden border-[0.5px] border-white/10 relative">
                  <motion.div animate={{ width: `${percent}%` }} className={clsx("h-full rounded-full relative overflow-hidden", category.color)} />
                </div>
              </div>
            );
          })}
      ...
      <GlassCard className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            Gastos Fixos <span className="text-xs font-normal text-white/50 bg-white/10 px-2 py-1 rounded-full">R$ {totalFixed.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
          </h2>
          ...
          {fixedExpenses.map((expense) => {
            ...
            return (
              <div key={expense.id} className="relative group flex flex-col items-center gap-2">
                {editingFixed && <button onClick={() => handleDeleteFixed(expense.id)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white z-10"><X size={12} /></button>}
                <div className={clsx("w-12 h-12 rounded-2xl border flex items-center justify-center bg-white/5", expense.color)}><Icon size={20} /></div>
        ...
        {editingFixed && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden">
            {!addingFixed ? (
              <button onClick={() => setAddingFixed(true)} className="w-full py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white/70 hover:text-white transition-all flex items-center justify-center gap-2"><Plus size={16} /> <span className="text-sm">Adicionar Categoria</span></button>
            ) : (
              <div className="bg-white/5 border border-white/10 rounded-xl p-3 space-y-3">
                <input type="text" value={newFixed.name} onChange={(e) => setNewFixed({ ...newFixed, name: e.target.value })} placeholder="Nome" className="w-full bg-white/5 border border-white/10 rounded-lg py-2 px-3 text-sm text-white outline-none focus:border-fuchsia-500/50 transition-all placeholder:text-white/30" />
                ...
                <div className="grid grid-cols-6 gap-2">
                  {iconOptions.map((option, index) => {
                    ...
                    return (
                      <button key={index} onClick={() => setNewFixed({ ...newFixed, iconIndex: index })} className={clsx("w-10 h-10 rounded-lg flex items-center justify-center transition-all", newFixed.iconIndex === index ? "bg-fuchsia-500/30 border-fuchsia-400/50 border" : "bg-white/5 border border-white/10", option.color)}><Icon size={16} /></button>
                    );
                  })}
                </div>
                <div className="flex gap-2">
                  <button onClick={handleAddFixed} className="flex-1 py-2 rounded-lg bg-gradient-to-r from-orange-500 to-fuchsia-600 text-white text-sm font-medium transition-all">Adicionar</button>
